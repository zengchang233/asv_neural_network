...
from model import ResNet, OnlineTripletLoss
...
def train(epoch, model, criterion, optimizer, train_loader):#训练轮数，模型，loss，优化器，数据集读取
    ...
    for batch_idx, (data, label) in progress_bar:
        ...
        out, embedding = model(data)#通过模型，输出最后一层和倒数第二层

        loss, not_zero_triplets = criterion(embedding, label)#loss
        optimizer.zero_grad()
        loss.backward()#bp训练
        optimizer.step()
...
def main():
    ...
    batch_sampler = BalancedBatchSampler(train_dataset.labels, train_dataset.count, 10, 10)
    print('Num of classes: {}'.format(n_classes))

    model = ResNet(layers=[1, 1, 1], embedding_size=args.embedding_size, n_classes=n_classes)
    model.to(device)

...
def main():
    ...
    model = ResNet(layers=[1, 1, 1], embedding_size=args.embedding_size, n_classes=n_classes, s = args.s, m=args.m).to(device)
    ...
    print('thr : {:.4f}'.format(thresh))
    acc = (y_pred > thresh).sum() / len(y_true)
